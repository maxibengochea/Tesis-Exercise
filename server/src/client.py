...
class Client:
  ...
  def _create_private_key(self):
    ...
    os.makedirs(f'server/{self._common_name}', exist_ok=True)

    with open(os.path.join(f'server/{self._common_name}', "private_key.pem"), "wb") as f:
  ...
  def issue_csr(self):
    ...
    csr_path = os.path.join(f"server/{self._common_name}", "csr.pem")
